...
extension UIColor {
    class func colorWithHex(hex:UInt32, alpha:Float = 1.0) -> UIColor {
        ...
        return self(red: red, green: green, blue: blue, alpha: nAlpha)
    }
    ...
    class func colorWithCSSName(name:String) -> UIColor {
        let nameValue = name.lowercaseString
        if nameValue == "transparent" {
            return clearColor()
        }
        let rgb = listNameOfColors[name.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet())]
        if !rgb!.isEmpty {
            return colorWithRGB(rgb!)
        }
        return self()
    }
    ...
    class func colorWithRGB(rgb:String) -> UIColor {
        ...
        if values.count == 3 || hasAlpha {
            ...
            return self(hue: hue/255.0, saturation: saturation/255.0, brightness: brightness/255.0, alpha: alpha)
